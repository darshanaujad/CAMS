...
import { User, Mail, Phone, MapPin, Calendar, Upload, Book, Shield, Send, Loader2 } from 'lucide-react';
...
const StudentRegistration = () => {
    ...
    const handleFileChange = async (e, fieldPath) => {
        ...
        if (file) {
            ...
            reader.onloadend = () => {
                ...
                if (fieldPath === 'profilePhoto') {
                    ...
                    const [_, docName] = fieldPath.split('.');
                    setFormData(prev => ({
                        ...prev,
                        documents: {
                            ...prev.documents,
                            [docName]: base64String
                        }
                    }));
                }
            };
            ...
            <div className="max-w-5xl mx-auto">
                <div className="text-center mb-10">
                    ...
                    <p className="mt-4 text-lg text-gray-600">
                        Join our academic management system. Please fill in your details accurately.
                    </p>
                </div>
                    ...
                    <form onSubmit={handleSubmit} className="p-8 sm:p-12">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <InputGroup label="Full Name" icon={User}>
                                <input required type="text" name="name" value={formData.name} onChange={handleChange}
                                    className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all outline-none" placeholder="John Doe" />
                            </InputGroup>
                            <InputGroup label="Username" icon={User}>
                                <input required type="text" name="username" value={formData.username} onChange={handleChange}
                                    className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all outline-none" placeholder="johndoe123" />
                            </InputGroup>
                            <InputGroup label="Email Address" icon={Mail}>
                                <input required type="email" name="email" value={formData.email} onChange={handleChange}
                                    className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all outline-none" placeholder="john@example.com" />
                            </InputGroup>
                            <InputGroup label="Phone Number" icon={Phone}>
                                <input required type="tel" name="phone" value={formData.phone} onChange={handleChange}
                                    className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all outline-none" placeholder="+1 234 567 8900" />
                            </InputGroup>
                            <InputGroup label="Roll Number" icon={User}>
                                <input required type="text" name="rollNo" value={formData.rollNo} onChange={handleChange}
                                    className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all outline-none" placeholder="CS-2023-001" />
                            </InputGroup>
                            <InputGroup label="Profile Photo" icon={Upload}>
                                <div className="relative border-2 border-dashed border-gray-300 rounded-lg p-4 hover:bg-gray-50 transition-colors">
                                    <input type="file" accept="image/*" onChange={(e) => handleFileChange(e, 'profilePhoto')} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                                    <div className="flex flex-col items-center justify-center text-gray-500">
                                        {formData.profilePhoto ? <span className="text-green-600 font-medium">Image Selected</span> : <span>Click to Upload Profile Photo</span>}
                                    </div>
                                </div>
                            </InputGroup>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <InputGroup label="Date of Birth" icon={Calendar}>
                                <input required type="date" name="dob" value={formData.dob} onChange={handleDateChange}
                                    className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all outline-none" />
                            </InputGroup>
                            <InputGroup label="Age" icon={Calendar}>
                                <input type="number" name="age" value={formData.age} readOnly
                                    className="w-full px-4 py-3 rounded-lg border border-gray-300 bg-gray-100 cursor-not-allowed outline-none" placeholder="Auto-calculated" />
                            </InputGroup>
                            ...
                            <div className="md:col-span-2">
                                <InputGroup label="Street Address" icon={MapPin}>
                                    <input type="text" name="address.street" value={formData.address.street} onChange={handleChange}
                                        className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all outline-none" placeholder="123 Main St" />
                                </InputGroup>
                            </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <InputGroup label="Department" icon={Book}>
                                <input required type="text" name="department" value={formData.department} onChange={handleChange}
                                    className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all outline-none" placeholder="Computer Science" />
                            </InputGroup>
                            <InputGroup label="Year" icon={Book}>
                                <input required type="number" name="year" value={formData.year} onChange={handleChange}
                                    className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all outline-none" placeholder="1" />
                            </InputGroup>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {['tenthMarksheet', 'twelfthMarksheet', 'aadhaarCard', 'casteCertificate'].map((doc) => (
                                <InputGroup key={doc} label={doc.replace(/([A-Z])/g, ' $1').trim().toUpperCase()} icon={Upload}>
                                    <div className="relative border-2 border-dashed border-gray-300 rounded-lg p-4 hover:bg-gray-50 transition-colors">
                                        <input type="file" required={['tenthMarksheet', 'twelfthMarksheet'].includes(doc)} onChange={(e) => handleFileChange(e, `documents.${doc}`)} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                                        <div className="flex flex-col items-center justify-center text-gray-500">
                                            {formData.documents[doc] ? <span className="text-green-600 font-bold">File Attached</span> : <span>Upload {doc}</span>}
                                        </div>
                                    </div>
                                </InputGroup>
                            ))}
